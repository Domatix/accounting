...
class iva_record_wizzard(models.TransientModel):
    ...
    def get_data(self):
        if self.partner_type == 'supplier':
            invoice_tax_obj = self.env['account.invoice.tax'].search([('invoice_id.date', '>=', self.date_start), ('invoice_id.date', '<=', self.date_end), ('company_id', '=', self.company_id.id), ('invoice_id.state', 'in', ['open', 'paid']), ('invoice_id.type', 'in', ['in_invoice', 'in_refund'])])
        elif self.partner_type == 'client':
            invoice_tax_obj = self.env['account.invoice.tax'].search([('invoice_id.date', '>=', self.date_start), ('invoice_id.date', '<=', self.date_end), ('company_id', '=', self.company_id.id), ('invoice_id.state', 'in', ['open', 'paid']), ('invoice_id.type', 'in', ['out_invoice', 'out_refund'])])
        else:
            invoice_tax_obj = self.env['account.invoice.tax'].search([('invoice_id.date', '>=', self.date_start), ('invoice_id.date', '<=', self.date_end)])

        positive = invoice_tax_obj.filtered(lambda r: r.invoice_id.amount_untaxed_signed > 0 and 'igic' not in r.name and 'IVA' in r.name)
        negative = invoice_tax_obj.filtered(lambda r: r.invoice_id.amount_untaxed_signed < 0 and 'igic' not in r.name and 'IVA' in r.name)
        ...
        if self.igic:
            igic_negative = invoice_tax_obj.filtered(lambda r: r.invoice_id.amount_untaxed_signed < 0 and 'igic' in r.name and 'IVA' not in r.name)
            igic_positive = invoice_tax_obj.filtered(lambda r: r.invoice_id.amount_untaxed_signed > 0 and 'igic' in r.name and 'IVA' not in r.name)
